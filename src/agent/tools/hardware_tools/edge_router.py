"""
Unified transport layer for lab bridge communication.

All device tools (xarm, abb, plc, network) use send_command() from this module.
In MOCK_MODE, dispatches to local mock handlers. In real mode, sends via WebSocket.

Config env vars:
  MOCK_MODE=true|false       (default: true)
  LAB_BRIDGE_URL=wss://...   (WebSocket URL)

Mock mode is switchable at runtime via set_mock_mode(bool).
"""
...
def _dispatch_ws(
    ...
    try:
        ...
        # Pack routing info into params for the lab_bridge dispatcher
...
def send_command_multi(
    ...
    Returns:
        dict mapping device_id to response
    """
