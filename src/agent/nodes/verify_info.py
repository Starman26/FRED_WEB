...
User identity verification node. Extracts identifiers (ID, email, phone)
from messages and verifies against Supabase before granting access to
personal data.
"""
import os
import re
...
def get_customer_id_from_identifier(
    ...
    """Resolve identifier to (customer_id, customer_data) or (None, None)."""
    if not identifier:
        return None, None
...
def verify_info_node(state: AgentState) -> Dict[str, Any]:
    """Verify user identity. Extracts identifier from message, checks DB,
    or triggers HITL to request credentials."""
...
def needs_verification(state: AgentState, message: str) -> bool:
    """Check if request needs identity verification (personal data keywords)."""
    if state.get("customer_id"):
        return False
